...
function __direct_weapon_dev(bData, isUpdate, _cb) {
    if (!isUpdate) {
        bData.ellipse = {
            heightReference: Cesium.HeightReference.CLAMP_TO_GROUND,
            clampToGround: true,
    ...
    // bData.polyline = {
    //     positions: [bData.position_copy, endPos],
    ...
    var height_start = Number(positionCarto.height);

    var positionCarto2 = Cesium.Cartographic.fromCartesian(endPos);
    ...
    var height_stop = Number(positionCarto2.height);
    ...
    Cesium.sampleTerrainMostDetailed(viewer.terrainProvider, cartographic).then((raisedPositionsCartograhpic) => {
        ...
        // let _glowm = new Cesium.PolylineGlowMaterialProperty({
        //     glowPower: 0.25,
        //     color: Cesium.Color.fromCssColorString(`#F4CA16`).withAlpha(0.7) //;Cesium.Color.fromCssColorString('#00f').withAlpha(0.9)
        // });
        ...
        // var _greenPolyMat = new Cesium.CircleWaveMaterialProperty({
        //     duration: 5000,
        //     gradient: 0,
        //     color: Cesium.Color.fromCssColorString("#3FE769").withAlpha(
        //         0.5), //new Cesium.Color(1.0, 0.0, 0.0, 0.1),
        //     count: 1
        // });
        // // var _greenPolyMat = new Cesium.EllipsoidFadeMaterialProperty(Cesium.Color.ORANGE, 2000);

        if (isSeen == true) {
            var visiableLine_Positions = Cesium.Ellipsoid.WGS84.cartographicArrayToCartesianArray(cartographic_lerp);
            // console.log("Seen", visiableLine_Positions)
            bData.polyline = {
            ...
            // var rd = _computeCircleRadius3D([bData.position_copy, visiableLine_Positions[visiableLine_Positions.length - 1]])
            // var pnts = _computeCirclePolygon2(bData.position_copy, rd);
            // // var pHierarchy = new Cesium.PolygonHierarchy(pnts);
            // bData.polygon = new Cesium.PolygonGraphics({
            //     hierarchy: new Cesium.PolygonHierarchy(pnts),
            //     material: _greenPolyMat,
            //     fill: true,
            //     outline: true,
            //     outlineWidth: 15,
            ...
            // console.log("Not Seen", inLine_Positions)
            bData.polyline = {
            ...
            // var rd = _computeCircleRadius3D([bData.position_copy, inLine_Positions[inLine_Positions.length - 1]])
            // var pnts = _computeCirclePolygon2(bData.position_copy, rd);
            // // var pHierarchy = new Cesium.PolygonHierarchy(pnts);
            // bData.polygon = new Cesium.PolygonGraphics({
            //     hierarchy: new Cesium.PolygonHierarchy(pnts),
    ...
    // bData.polyline = {
    //     positions: [bData.position, endPos],
    //     material: new Cesium.PolylineGlowMaterialProperty({
    //         glowPower: 0.16,
    //         color: Cesium.Color.GREEN
    //     }),
