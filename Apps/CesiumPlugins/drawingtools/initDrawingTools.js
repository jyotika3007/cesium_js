            ...
            if(Array.isArray(_weaponsList[p])){
                _weaponsList[p].forEach(w => {
                    if(p && !drawnedDivs[p]){
                        _created = true;
                        drawnedDivs[p] = true;
                        iconsStrs += '<div class="wpn_list_dynamic" style="display: flex;flex-direction: column;border: 1px solid gray;margin-top: 4px;border-radius: 5px;background: #00000038;margin-right: 4px;">';
...
function showPolygon(objId, positions) {
    var post_array = {};
    for(ik=0; ik<positions.length; ik++){
        post_array[ik] = positions[ik];
    }
    ...
    var bData = {
        ...
        objId: objId,
        shapeType: "Polygon",
        positionpolygon : post_array,
        polygon_color: selected_color,
        polyline_color: selected_outline_color,
        polyline: {
...
function showCircle(objId, positions) {
    ...
    // });
    wallAnimPropsDic[objId]["position"] = positions[0];
...
function updatePoint() {
    ...
    if (selected_Entity !== undefined && selected_Entity.objId !== undefined && wallAnimPropsDic[selected_Entity.objId] !== undefined) {
        // console.log("selected_Entity.viewModel", selected_Entity.viewModel);
        // console.log("selected")
        let transculency = undefined;
        if (wallAnimPropsDic[selected_Entity.objId]["imgTranslucency"]) {
            transculency = new Cesium.NearFarScalar(1.5e2, 2.0, 1.5e5, 0);
        }
        if (selected_Entity.viewModel["flat"]) {
            var positions = [];
            var position = selected_Entity.position._value;
            positions.push({
                x: position.x - (2 * selected_Entity.viewModel.scale),
                y: position.y - (2 * selected_Entity.viewModel.scale),
                z: position.z
            });
            positions.push({
                x: position.x + (2 * selected_Entity.viewModel.scale),
            ...
            selected_Entity.rectangle = {
                coordinates: Cesium.Rectangle.fromCartesianArray(positions),
                material: wallAnimPropsDic[selected_Entity.objId]["img"],
                heightReference: Cesium.HeightReference.CLAMP_TO_GROUND,
                rotation: Cesium.Math.toRadians(Number(selected_Entity.viewModel.xrotate)),
                stRotation: Cesium.Math.toRadians(Number(selected_Entity.viewModel.xrotate)),
                clampToGround: true
            };

            selected_Entity.label = {
               horizontalOrigin: Cesium.HorizontalOrigin.LEFT,
               verticalOrigin: Cesium.VerticalOrigin.BOTTOM,
               translucencyByDistance: transculency,
            pixelOffset: new Cesium.Cartesian2(-20, -45) //left top
        }


        selected_Entity.billboard = undefined;

    } else {
        var heading = Cesium.Math.toRadians(Number(selected_Entity.viewModel.xrotate));
        var pitch = Cesium.Math.toRadians(Number(selected_Entity.viewModel.yrotate));
        ...
        selected_Entity.orientation = orientation;

        selected_Entity.billboard = {
            image: wallAnimPropsDic[selected_Entity.objId]["img"],
            scale: selected_Entity.viewModel.scale,
            translucencyByDistance: transculency,
            ...
            // sizeInMeters: wallAnimPropsDic[selected_Entity.objId]["imgSizeInMeters"],
            show: true
        };

            //  selected_Entity.billboard = {
            //     dimensions: new Cesium.Cartesian3(8 * selected_Entity.viewModel.scale, 0.0, 8 * selected_Entity.viewModel.scale),
            //     //material: new Cesium.ImageMaterialProperty({ image: wallAnimPropsDic[selected_Entity.objId]["img"], transparent: true }),
            //     material: wallAnimPropsDic[selected_Entity.objId]["img"],
            //     outline: false,
            //     clampToGround: true,
            //     heightReference: Cesium.HeightReference.CLAMP_TO_GROUND
...
selected_Entity.label = {
            ...
            selected_Entity.rectangle = undefined;
        }
    }
}
...
function showPoint(objId, position, latLon) {
    ...
    if (bData.viewModel === undefined) {
        bData.viewModel = {
            "scale": wallAnimPropsDic[objId]["scale"],
            "flat": wallAnimPropsDic[objId]["flat"],
            "xrotate": 0.0,
            "yrotate": 0.0,
        };
    }
    ...
    if (wallAnimPropsDic[objId]["flat"]) {
        var positions = [];
        positions.push({
            x: position.x - (2 * wallAnimPropsDic[objId]["scale"]),
            y: position.y - (2 * wallAnimPropsDic[objId]["scale"]),
            z: position.z
        });
        positions.push({
            x: position.x + (2 * wallAnimPropsDic[objId]["scale"]),
        ...
        // bData.box = {
        //     dimensions: new Cesium.Cartesian3(8 * wallAnimPropsDic[objId]["scale"], 0.0, 8 * wallAnimPropsDic[objId]["scale"]),
        //     // material: new Cesium.ImageMaterialProperty({ image: wallAnimPropsDic[objId]["img"]}),
        //     material: wallAnimPropsDic[objId]["img"],
        //     outline: false,
        //     clampToGround: true,
        //     heightReference: Cesium.HeightReference.CLAMP_TO_GROUND
    ...
    if (wallAnimPropsDic[objId]["displayInfo"] !== undefined) {
        // alert("Getting Info")
        console.log(wallAnimPropsDic[objId]);
        symbol_desc = wallAnimPropsDic[objId]["displayInfo"];
        bData.displayInfo = wallAnimPropsDic[objId]["displayInfo"];
        bData.displayInfo["category"] = wallAnimPropsDic[objId]["category"];
        color_code_symbol = Cesium.Color.fromCssColorString('#555555')
        if( wallAnimPropsDic[objId]["category"] == "Blue"){
            color_code_symbol = Cesium.Color.fromCssColorString('#0000ff');
        }
        if( wallAnimPropsDic[objId]["category"] == "Red"){
            color_code_symbol = Cesium.Color.fromCssColorString('#ff0000');
        }
        if( wallAnimPropsDic[objId]["category"] == "Yellow"){
            color_code_symbol = Cesium.Color.fromCssColorString('#ffff00');
        }

        bData.label = {
            text: symbol_desc.title,
            show: true,
            showBackground: true,
            backgroundColor :color_code_symbol,
            font: '14px sans-serif',
            horizontalOrigin: Cesium.HorizontalOrigin.LEFT,
            verticalOrigin: Cesium.VerticalOrigin.BOTTOM,
            translucencyByDistance: transculency,
...
function showWeapon(objId, position, latLon) {
    ...
    if (wallAnimPropsDic[objId]["flat"]) {
        ...
        if(wallAnimPropsDic[objId]["WPN_EQPT"] !== undefined && wallAnimPropsDic[objId]["WPN_EQPT"].toLowerCase() == "rlos" ){
            bData.billboard = {
                image: wallAnimPropsDic[objId]["img"],
                scale: wallAnimPropsDic[objId]["scale"],
                    // translucencyByDistance: transculency,
                ...
                bData.box = {
                    dimensions: new Cesium.Cartesian3(8 * wallAnimPropsDic[objId]["scale"], 0.0, 8 * wallAnimPropsDic[objId]["scale"]),
                    material: new Cesium.ImageMaterialProperty({ image: wallAnimPropsDic[objId]["img"], transparent: true }),
                    outline: false,
                    clampToGround: true,
                    heightReference: Cesium.HeightReference.CLAMP_TO_GROUND
                }
    ...
    function updateText() {
        ...
        // console.log("selected_Entity.viewModel", selected_Entity.viewModel);
        console.log(1)
        // var imgg = Cesium.writeTextToCanvas(selected_Entity.viewModel.text, { font: `${selected_Entity.viewModel.scale}px sans-serif` });
        // var imgg = Cesium.writeTextToCanvas(selected_Entity.viewModel.text, {
        //     backgroundColor: new Cesium.Color.fromCssColorString("rgba(0.8, 0.6, 0.8, 0.8)"),
        //     padding: 2,
        //     fill: true,
        //     font: `${selected_Entity.viewModel.scale}px sans-serif`,
        //     stroke: true,
        //     strokeColor: Cesium.Color.BLACK
        // });

        wallAnimPropsDic[selected_Entity.objId].bgColorRed = selected_Entity.viewModel.bgColorRed
        wallAnimPropsDic[selected_Entity.objId].bgColorGreen = selected_Entity.viewModel.bgColorGreen
        wallAnimPropsDic[selected_Entity.objId].bgColorBlue = selected_Entity.viewModel.bgColorBlue
        wallAnimPropsDic[selected_Entity.objId].bgColorAlpha = selected_Entity.viewModel.bgColorAlpha
        wallAnimPropsDic[selected_Entity.objId].strokeColorRed = selected_Entity.viewModel.strokeColorRed
        wallAnimPropsDic[selected_Entity.objId].strokeColorGreen = selected_Entity.viewModel.strokeColorGreen
        wallAnimPropsDic[selected_Entity.objId].strokeColorBlue = selected_Entity.viewModel.strokeColorBlue
        wallAnimPropsDic[selected_Entity.objId].strokeColorAlpha = selected_Entity.viewModel.strokeColorAlpha
        wallAnimPropsDic[selected_Entity.objId].fillColorRed = selected_Entity.viewModel.fillColorRed
        wallAnimPropsDic[selected_Entity.objId].fillColorGreen = selected_Entity.viewModel.fillColorGreen
        wallAnimPropsDic[selected_Entity.objId].fillColorBlue = selected_Entity.viewModel.fillColorBlue
        wallAnimPropsDic[selected_Entity.objId].fillColorAlpha = selected_Entity.viewModel.fillColorAlpha
        wallAnimPropsDic[selected_Entity.objId].fill = selected_Entity.viewModel.fill
        wallAnimPropsDic[selected_Entity.objId].font = selected_Entity.viewModel.font
        wallAnimPropsDic[selected_Entity.objId].stroke = selected_Entity.viewModel.stroke

        var imgg = Cesium.writeTextToCanvas(selected_Entity.viewModel.text, {
            backgroundColor: new Cesium.Color.fromCssColorString(`rgba(${selected_Entity.viewModel.bgColorRed}, ${selected_Entity.viewModel.bgColorGreen}, ${selected_Entity.viewModel.bgColorBlue}, ${selected_Entity.viewModel.bgColorAlpha})`),
            padding: 2,
            fill: selected_Entity.viewModel.fill,
            font: `${selected_Entity.viewModel.scale}px ${selected_Entity.viewModel.font}`,
            stroke: selected_Entity.viewModel.stroke,
            strokeColor: new Cesium.Color.fromCssColorString(`rgba(${selected_Entity.viewModel.strokeColorRed}, ${selected_Entity.viewModel.strokeColorGreen}, ${selected_Entity.viewModel.strokeColorBlue}, ${selected_Entity.viewModel.strokeColorAlpha})`),
            fillColor: new Cesium.Color.fromCssColorString(`rgba(${selected_Entity.viewModel.fillColorRed}, ${selected_Entity.viewModel.fillColorGreen}, ${selected_Entity.viewModel.fillColorBlue}, ${selected_Entity.viewModel.fillColorAlpha})`),
        });
        ...
        if (selected_Entity.viewModel["flat"]) {
            ...
            var position = selected_Entity.position._value;
            positions.push({
                x: position.x - (imgg.width),
                y: position.y - (imgg.height),
                z: position.z
            });
            positions.push({
                x: position.x + (imgg.width),
...
function showText(objId, position, text) {
    ...
    if (!Cesium.defined(wallAnimPropsDic[objId])) {
        wallAnimPropsDic[objId] = { 
            displayInfo: {
                "title": "Text"
            }, 
    ...
    if (!Cesium.defined(wallAnimPropsDic[objId]["scale"])) {
        wallAnimPropsDic[objId]["scale"] = 20;
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["flat"])) {
        wallAnimPropsDic[objId]["flat"] = true;
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["font"])) {
        wallAnimPropsDic[objId]["font"] = "sans-serif";
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["fill"])) {
        wallAnimPropsDic[objId]["fill"] = true;
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["bgColorRed"])) {
        wallAnimPropsDic[objId]["bgColorRed"] = 1.0;
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["bgColorGreen"])) {
        wallAnimPropsDic[objId]["bgColorGreen"] = 1.0;
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["bgColorBlue"])) {
        wallAnimPropsDic[objId]["bgColorBlue"] = 1.0;
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["bgColorAlpha"])) {
        wallAnimPropsDic[objId]["bgColorAlpha"] = 0.8;
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["fillColorRed"])) {
        wallAnimPropsDic[objId]["fillColorRed"] = 255;
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["fillColorGreen"])) {
        wallAnimPropsDic[objId]["fillColorGreen"] = 255;
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["fillColorBlue"])) {
        wallAnimPropsDic[objId]["fillColorBlue"] = 255;
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["fillColorAlpha"])) {
        wallAnimPropsDic[objId]["fillColorAlpha"] = 1.0;
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["stroke"])) {
        wallAnimPropsDic[objId]["stroke"] = true;
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["strokeColorRed"])) {
        wallAnimPropsDic[objId]["strokeColorRed"] = 0.0;
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["strokeColorGreen"])) {
        wallAnimPropsDic[objId]["strokeColorGreen"] = 0.0;
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["strokeColorBlue"])) {
        wallAnimPropsDic[objId]["strokeColorBlue"] = 0.0;
    }
    if (!Cesium.defined(wallAnimPropsDic[objId]["strokeColorAlpha"])) {
        wallAnimPropsDic[objId]["strokeColorAlpha"] = 1.0;
    }

    // var imgg = Cesium.writeTextToCanvas(text, {
    //     backgroundColor: new Cesium.Color.fromCssColorString("rgba(0.8, 0.6, 0.8, 0.8)"),
    //     padding: 2,
    //     fill: true,
    //     font: `${wallAnimPropsDic[objId]["scale"]}px sans-serif`,
    //     stroke: true,
    //     strokeColor: Cesium.Color.BLACK
    // });

    var imgg = Cesium.writeTextToCanvas(text, {
        backgroundColor: new Cesium.Color.fromCssColorString(`rgba(${wallAnimPropsDic[objId]["bgColorRed"]}, ${wallAnimPropsDic[objId]["bgColorGreen"]}, ${wallAnimPropsDic[objId]["bgColorBlue"]}, ${wallAnimPropsDic[objId]["bgColorAlpha"]})`),
        padding: 2,
        fill: wallAnimPropsDic[objId]["fill"],
        fillColor: new Cesium.Color.fromCssColorString(`rgba(${wallAnimPropsDic[objId]["fillColorRed"]}, ${wallAnimPropsDic[objId]["fillColorGreen"]}, ${wallAnimPropsDic[objId]["fillColorBlue"]}, ${wallAnimPropsDic[objId]["fillColorAlpha"]})`),
        font: `${wallAnimPropsDic[objId]["scale"]}px ${wallAnimPropsDic[objId]["font"]}`,
        stroke: wallAnimPropsDic[objId]["stroke"],
        strokeColor: new Cesium.Color.fromCssColorString(`rgba(${wallAnimPropsDic[objId]["strokeColorRed"]}, ${wallAnimPropsDic[objId]["strokeColorGreen"]}, ${wallAnimPropsDic[objId]["strokeColorBlue"]}, ${wallAnimPropsDic[objId]["strokeColorAlpha"]})`),
    });
    ...
    if (bData.viewModel === undefined) {
        ...
        bData.viewModel = {
            ...
            "fill": wallAnimPropsDic[objId]["fill"],
            "stroke": wallAnimPropsDic[objId]["stroke"],
            "bgColorRed": wallAnimPropsDic[objId]["bgColorRed"],
            "bgColorGreen": wallAnimPropsDic[objId]["bgColorGreen"],
            "bgColorBlue": wallAnimPropsDic[objId]["bgColorBlue"],
            "bgColorAlpha": wallAnimPropsDic[objId]["bgColorRed"],
            "strokeColorRed": wallAnimPropsDic[objId]["strokeColorRed"],
            "strokeColorGreen": wallAnimPropsDic[objId]["strokeColorGreen"],
            "strokeColorBlue": wallAnimPropsDic[objId]["strokeColorBlue"],
            "strokeColorAlpha": wallAnimPropsDic[objId]["strokeColorAlpha"],
            "fillColorRed": wallAnimPropsDic[objId]["fillColorRed"],
            "fillColorGreen": wallAnimPropsDic[objId]["fillColorGreen"],
            "fillColorBlue": wallAnimPropsDic[objId]["fillColorBlue"],
            "fillColorAlpha": wallAnimPropsDic[objId]["fillColorRed"],
            "font": wallAnimPropsDic[objId]["font"]
        };
    ...
    if (wallAnimPropsDic[objId]["flat"]) {
        ...
        bData.box = {
            dimensions: new Cesium.Cartesian3(imgg.width, 0.0, imgg.height),
            material: new Cesium.ImageMaterialProperty({
                image: imgg
            }),
            outline: false,
...
// for formation Deployment

function _drawManualPoint(category, displayInfo, img) {
    var _htmlcontent = `
    <div style="font-family: sans-serif;font-size: small;color:white;">
    <table>
    <tbody>
    ...
    <input type="checkbox" checked onchange="default_image_translucency_changed(this);">
    ...
    showConfirmation(_htmlcontent, "Symbol Settings", function (success) {
        if (success) {
            flag = 0;
            let wd = {};
            wd["category"] = category;
            wd["displayInfo"] = displayInfo;
            wd["img"] = img;
            wd["scale"] = imgScale;
            wd["flat"] = isFlat;
            wd["imgTranslucency"] = imgTranslucency;
