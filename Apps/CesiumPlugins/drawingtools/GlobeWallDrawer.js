...
Cesium.Material._materialCache.addMaterial('Wallpaper', {
    fabric: {
        ...
        components: {
            diffuse: 'texture2D(image, fract((gl_FragCoord.xy ) / vec2(imageDimensions.xy))).rgb',
            alpha: 'texture2D(image, fract((gl_FragCoord.xy ) / vec2(imageDimensions.xy))).a'
        }
    },
    translucent: false
});

var WallPaperMaterialProperty = function (image) {
    this._image = image;
...
var GlobeWallDrawer = function () {
    this.init.apply(this, arguments);
};

GlobeWallDrawer.prototype = {
    viewer: null,
    scene: null,
    clock: null,
    canvas: null,
    ...
    okHandler: null,
    cancelHandler: null,
    dragIcon: "CesiumPlugins/drawingtools/images/circle_gray.png",
    dragIconLight: "CesiumPlugins/drawingtools/images/circle_red.png",
    material: null,
    toolBarIndex: null,
    markers: {},
    ...
    showModifyWall: function (positions, okHandler, cancelHandler) {
        var _this = this;
        _this.positions = positions;
        _this.okHandler = okHandler;
        _this.cancelHandler = cancelHandler;
        _this._showModifyWall2Map();
    },
    startDrawWall: function (okHandler, cancelHandler) {
        var _this = this;
        _this.okHandler = okHandler;
        _this.cancelHandler = cancelHandler;
        ...
        _this.drawHandler.setInputAction(function (event) {
            ...
            if (_this.positions.length < 1) {
                _this.tooltip.showAt(position, "<p>Select a starting point</p>");
                return;
            }
            var num = _this.positions.length;
            var tip = "<p>Click to add next point</p>";
            if (num > 2) {
                tip += "<p>Right click to end drawing</p>";
            }
    ...
    _startModify: function () {
        ...
        _this.modifyHandler.setInputAction(function (event) {
            ...
            pickedAnchor = entity;
            if (typeof (_promptPositionConfirmation) == "function") {
                _promptPositionConfirmation(cartesian, function (success) {
                    if (success) {
                        cartesian = _newCart3Pos;
                        isMoving = false;
        ...
        _this.modifyHandler.setInputAction(function (event) {
            ...
            if (isMoving) {
                ...
                if (entity.flag == "anchor") {
                    _this.tooltip.showAt(position, "<p>Move control point</p>");
                }
                if (entity.flag == "mid_anchor") {
                    _this.tooltip.showAt(position, "<p>Move to create a new control point</p>");
                }
            }
        }, Cesium.ScreenSpaceEventType.LEFT_CLICK);
    ...
    _createToolBar: function () {
        ...
        var html = '<div id="shapeEditContainer" style="padding: 10px 10px;">' +
            '    <button name="btnOK" class="layui-btn layui-btn-xs layui-btn-normal ui-button ui-corner-all ui-widget"> <i class="layui-icon"></i> Confirm </button>' +
            '    <button name="btnCancel" class="layui-btn layui-btn-xs layui-btn-danger ui-button ui-corner-all ui-widget"> <i class="layui-icon">ဆ</i> Cancel </button>' +
