    ...
    showRainWeather(enabled) {
        if (enabled === true && Weather._rainFlag === null) {
            ...
            Weather._rainStage = Weather._viewer.scene.postProcessStages.add(new Cesium.PostProcessStage({
                "name": "rain",
                fragmentShader:
                    //  time float v=1.-sin(hash(floor(uv.x*100.))*4.);
                    //   float a=-.3;
...
// viewer.scene.skyBox = new Cesium.SkyBoxOnGround({
