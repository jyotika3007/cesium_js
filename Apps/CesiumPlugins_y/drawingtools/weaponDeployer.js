...
function change_theta(positionCBP, initialVelocity, vangle, hangle) {
    let centerPoint = Cesium.Cartographic.fromCartesian(positionCBP);

    let centerXYZ = Cesium.Cartographic.toCartesian(centerPoint);
    let enuTransform = Cesium.Transforms.eastNorthUpToFixedFrame(centerXYZ);

    /* second phase */
    // let verticalVelocity = initialVelocity * Math.sin(Cesium.Math.toRadians(vangle));
    // let horizontalVelocity = initialVelocity * Math.cos(Cesium.Math.toRadians(vangle));

    let maxHeight = Math.pow(initialVelocity, 2) * Math.pow(Math.sin(Cesium.Math.toRadians(vangle)), 2) / (2 * 9.8);

    // let sin = Math.sin(Cesium.Math.toRadians(vangle));
    // let  maxHeight = (initialVelocity * initialVelocity * sin * sin) / (9.8 * 2);
    // let maxHeight = (verticalVelocity * verticalVelocity) / (2 * 9.8);
    // let range0 = initialVelocity * initialVelocity * 2 * Math.sin(Cesium.Math.toRadians(vangle)) * Math.cos(Cesium.Math.toRadians(vangle)) / 9.8;

    // d = V₀² * sin(2 * α) / g

    let range = initialVelocity * initialVelocity * Math.sin(Cesium.Math.toRadians(vangle) * 2) / 9.8;
...
function _indirect_weapon_dev(bData, isUpdate, _cb) {
    var tVelocity = Number(bData.viewModel.Muzzle_Velocity);

    if (Number(bData.viewModel._elevation_angle) > bData.viewModel._up_elevation_angle) {
        bData.viewModel._elevation_angle = bData.viewModel._up_elevation_angle;
    }
    if (Number(bData.viewModel._elevation_angle) < bData.viewModel._down_elevation_angle) {
        bData.viewModel._elevation_angle = bData.viewModel._down_elevation_angle;
    }

    if (bData.tVelocity === undefined) {
        //tVelocity = _getBiasVelocity(Number(bData.viewModel.Muzzle_Velocity), Number(bData.viewModel._elevation_angle), Number(bData.viewModel.Planning_Rg));
        tVelocity = Math.sqrt((Number(bData.viewModel.Planning_Rg) * 9.8) / Math.sin(Cesium.Math.toRadians(Number(bData.viewModel._elevation_angle)) * 2));
        bData.tVelocity = tVelocity;
    } else {
        tVelocity = bData.tVelocity;
    }
    var yu = change_theta(bData.position_copy, Number(tVelocity), Number(bData.viewModel._elevation_angle), Number(bData.viewModel._direction_angle));
    var controls = [];
    ...
    Cesium.sampleTerrainMostDetailed(viewer.terrainProvider, cartographic).then((raisedPositionsCartograhpic) => {
        ...
        */
        ...
        // bData.box.stRotation = Cesium.Math.toRadians(Number(bData.viewModel._direction_angle) * -1);
            ...
            case "arty gun":
                if (!bData.viewModel) {
                    bData.viewModel = {
                        ...
                        _direction_angle: 0,
                        _elevation_angle: 45,
                        _up_elevation_angle: Number(weaponData.up_elevation),
                        _down_elevation_angle: Number(weaponData.down_elevation)
                    }
                }
