...
function _globalSearchChangePlaceholder() {
    switch (_globalSearchTypeItem.toLowerCase()) {
        case "location":
            $(".globalSearchTextbox").attr("placeholder", "Longitude, Latitude");
            break;
        case "symbol":
...
function _gotoSelectedPoint(pointLocation) {
    if (pointLocation && Array.isArray(pointLocation)) {
        console.log(pointLocation);
        viewer.camera.flyTo({
            destination: Cesium.Cartesian3.fromDegrees(pointLocation[0], pointLocation[1], 8000),
            duration: 6.0
        });
    }
}
...
function _globalGeoCoderSearch(text) {
    if (!text || text == "") {
        text = _globalSearchTypeText;
    }
    httpGetAsync(`http://localhost:2020/geocoder/${text}`, function (gjson) {
        if (gjson && gjson !== "") {
            let gjsonArray = JSON.parse(gjson);
            let globalSearchResultUl = $(".globalSearchResultUl");
            if (gjsonArray && gjsonArray.features && Array.isArray(gjsonArray.features)) {
                gjsonArray.features.forEach(g => {
                    if (g.properties && g.properties.label && g.geometry && g.geometry.type == "Point" && g.geometry.coordinates) {
                        globalSearchResultUl.append($(`<li>
                                    <div><span style="font-size: smaller;color: aqua;">Address</span><br>
                                        <span></span>&nbsp;<span style="color:lightcoral;">${g.properties.label}</span>
                                        <span onclick="_gotoSelectedPoint(${JSON.stringify(g.geometry.coordinates)})" style="background:cadetblue;padding:2px;border-radius:2px;font-size:smaller;cursor:pointer;">&#10148;</span>
                                    </div>
                                    <hr>
                                </li>`))
                    }
